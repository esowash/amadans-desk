r"""W_AmadanMenu SaveAndCloseButton::OnClicked -> AddKeepRule -> Close.

Identical chain to build_amadanmenu_saverule_onclicked.py (SaveButton), with one addition: a
Close() call chained after AddKeepRule.then, cloned VERBATIM from the real, original, working
Close node on Button_634 (RAW_CLOSE_CALL below -- self pin visible but intentionally left
unwired, exactly as captured; both the pre- and post-cleanup pulls of Button_634's own chain show
this same unlinked self pin compiling and running fine, so it's cloned unmodified rather than
guessed at).
...
# Cloned verbatim from Button_634's real, original, working Close node (both the pre- and
# post-cleanup pulls of that chain show this exact shape -- self pin visible, PinSubCategoryObject
# WindowRoot, bHidden=False, yet intentionally left UNWIRED -- and it compiled and ran fine both
# times, so no wire is added here either, matching the real proven config exactly).
...
boundEvent._replace_prop(
    "CustomFunctionName",
    '"BndEvt__W_AmadanMenu_SaveAndCloseButton_K2Node_ComponentBoundEvent_2_OnButtonClickedEvent__DelegateSignature"',
)
...
# --- Close (chained after AddKeepRule, cloned verbatim from Button_634's real working node) -----
...
closeCall.set_position(2150, 300)
...
out = MOD + r"\.ccmod\graphs\amadanmenu_saveandclose_onclicked_body.t3d"
