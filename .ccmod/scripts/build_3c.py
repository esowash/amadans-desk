"""Transform stocker_3b.t3d -> stocker_3c.t3d.

Swap the no-op move mechanism (VariableGet[PlaceableInventory] proxy +
MoveAndStackItems on ItemDistributor) for the finalized recipe:
  invN = GetInventoryByType(chestN, PlaceableInventory)   # real ItemInventory
  srcInv.MoveItemsByTemplateId(templateID, 0, bMoveAllAvailable=true, dstInv, ignoreSizeLimit=true)
Selection front-half (BeginPlay->Delay->GetAllActors->GetArrayItem->
GetPopulatedItemCount->Greater->Branch) is preserved from 3B unchanged.
"""
import sys, re
import os, pathlib
...
# --- log banners: mark 3C + new mechanism (pak-grep sentinel) ------------
set_default(N("K2Node_CallFunction_0"),  "InString", "STOCKER_3C_GIBT begin")
set_default(ps30, "InString", "STOCKER_3C src=chest0 (MIBT 12515 -> chest1)")
set_default(ps40, "InString", "STOCKER_3C src=chest1 (MIBT 12515 -> chest0)")
...
out = MOD + r"\.ccmod\graphs\stocker_3c.t3d"
