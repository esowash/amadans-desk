"""Author stocker_3v.t3d -- friendly-name spike: GetBuildableObjectName for
each managed station.

Operates on the FULL live graph (pull-edit-repaste), per stocker-full-graph-edits.

Pinned lead from earlier this session, resumed: GetDisplayName gives
<ClassName>_<instanceId> (session-volatile, not what the player sees).
BuildingFunctionLibrary::GetBuildableObjectName (static, generic Object
input, Text output) is the real mechanism -- this spike directly tests
whether it distinguishes "Furnace" from "Improved Furnace" for
BP_PL_CraftingStation_Furnace vs _Furnace2, the concrete example that started
this whole investigation.

Read-only, purely additive -- appended after 3P's true tail. ForEach
ManagedStations -> GetBuildableObjectName(Object=station) -> Conv_TextToString
-> Print, alongside GetDisplayName for direct side-by-side comparison.
"""
import sys
...
row = conn.execute("SELECT t3d FROM graphs WHERE name=?", ("stocker_3v_base",)).fetchone()
assert row, "run `ccmod pull --save stocker_3v_base` first"
g = parse(row[0])
...
GBON_T = tmpl_file(LIB + r"\actor\get_buildable_object_name.t3d")
...
# --- entry -------------------------------------------------------------------
pLbl = add(PRINT_T)
setd(pLbl, "InString", "STOCKER_3V begin (friendly-name spike)")

stationsGet = add(GET_STATIONS_T)
fe = add(FEACH_T)
type_foreach(fe, bgc(BASE_CONTAINER))
connect(stationsGet, "ManagedStations", fe, "Array")

gdn = add(GDN_T)
connect(fe, "Array Element", gdn, "Object")
pInternal = add(PRINT_T)
setd(pInternal, "InString", "STOCKER_3V internal=")
connect(gdn, "ReturnValue", pInternal, "InString")

gbon = add(GBON_T)
connect(fe, "Array Element", gbon, "Object")
c2s = add(C2S_T)
connect(gbon, "Name", c2s, "InText")
pFriendly = add(PRINT_T)
setd(pFriendly, "InString", "STOCKER_3V friendly=")
connect(c2s, "ReturnValue", pFriendly, "InString")

pDone = add(PRINT_T)
setd(pDone, "InString", "STOCKER_3V done")

# --- exec ---------------------------------------------------------------
connect_exec(tail, pLbl)
connect_exec(pLbl, fe, "then", "Exec")
connect_exec(fe, pInternal, "LoopBody", "execute")
connect_exec(pInternal, pFriendly)
connect_exec(fe, pDone, "Completed", "execute")

# --- layout -----------------------------------------------------------------
pLbl.set_position(2400, 1900)
stationsGet.set_position(2400, 2050)
fe.set_position(2660, 1900)
gdn.set_position(2920, 1750); pInternal.set_position(3180, 1750)
gbon.set_position(2920, 2050); c2s.set_position(3180, 2050); pFriendly.set_position(3440, 2050)
pDone.set_position(2660, 2200)
...
out = MOD + r"\.ccmod\graphs\stocker_3v.t3d"
